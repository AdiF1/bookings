...
func (m *postgresDBRepo) InsertReservation(res models.Reservation) (int, error) {
	...
	// manipulate res.StartDate to have a 1500 hrs stamp and res.EndDate to have
	// a 1100 hrs stamp added to the dates passed before inserting to database

	res.StartDate = time.Date(res.StartDate.Year(), res.StartDate.Month(), res.StartDate.Day(), 15, 0, 0, 0, time.UTC)
	res.EndDate = time.Date(res.EndDate.Year(), res.EndDate.Month(), res.EndDate.Day(), 11, 0, 0, 0, time.UTC)
...
// Authenticate checks for valid user email / password
...
func (m *postgresDBRepo) UpdateReservation(usr models.Reservation) error {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
...
func (m *postgresDBRepo) UpdateProcessedForReservation(id, processedStatus int) error {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
	...
	_, err := m.DB.ExecContext(ctx, query, processedStatus, id)
	if err != nil {
		return err
	}
	return nil
