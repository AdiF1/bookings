...
var theTests = []struct {
	...
	{"esthers", "/esthers", "GET", http.StatusOK},
	{"sanc", "/sanctuary", "GET", http.StatusOK},
...
func TestRepository_Reservation(t *testing.T) {
	...
	// deal with tests that need access to session
	// create a new request
	req, _ := http.NewRequest("GET", "/make-reservation", nil)
	// get context with session header
	ctx := getCtx(req)
	// put the context into the new request
	req = req.WithContext(ctx)
	// init a new test response recorder
	rr := httptest.NewRecorder()
	// put the reservation with it's context into the session
	...
	// test case where reservation is NOT in session (reset everything). Same structure 
	// as above but without adding the reservation to the session
	req, _ = http.NewRequest("GET", "/make-reservation", nil)
	ctx = getCtx(req)
	req = req.WithContext(ctx)
	rr = httptest.NewRecorder()
	// NOT ADDING RESERVATION TO SESSION
	handler.ServeHTTP(rr, req)
...
func TestRepository_PostReservation(t *testing.T) {
	...
	postedData.Add("first_name", "Steve")
	postedData.Add("last_name", "Jobs")
	postedData.Add("email", "steve@jobs.com")
	...
	rr := httptest.NewRecorder()
	// convert Repo.PostReservation into a function so that it can call ServeHTTP
	...
	// repeat above test but for missing post body just by changing reqBody to nil
	...
	// test for invalid data (not enough first_name characters)
	postedData = url.Values{}
	postedData.Add("start_date", "2050-01-01")
	postedData.Add("end_date", "2050-01-02")
	postedData.Add("first_name", "S")
	postedData.Add("last_name", "Jobs")
	postedData.Add("email", "steve@jobs.com")
	postedData.Add("phone", "555-555-1234")
	postedData.Add("room_id", "1")
	...
	// test for failure to insert reservation into database (fails with room_id=2 as tested in testRepo)
	postedData = url.Values{}
	postedData.Add("start_date", "2050-01-01")
	postedData.Add("end_date", "2050-01-02")
	postedData.Add("first_name", "Steve")
	...
	if rr.Code != http.StatusTemporaryRedirect {
		t.Errorf("PostReservation handler failed when trying and failing to insert reservation: got %d, wanted %d", rr.Code, http.StatusTemporaryRedirect)
	}

	// test for failure to insert restriction into database (room_id as 1000)
	postedData = url.Values{}
	postedData.Add("start_date", "2050-01-01")
	postedData.Add("end_date", "2050-01-02")
	postedData.Add("first_name", "Steve")
...
func TestRepository_PostAvailability(t *testing.T) {
	...
	*****************************************/
	...
	if rr.Code != http.StatusSeeOther {
		t.Errorf("1-Post availability when no rooms available gave wrong status code: got %d, wanted %d", rr.Code, http.StatusSeeOther)
	}
	...
	*****************************************/
	...
	if rr.Code != http.StatusOK {
		t.Errorf("2-Post availability when rooms are available gave wrong status code: got %d, wanted %d", rr.Code, http.StatusOK)
	}
	...
	*****************************************/
	...
	if rr.Code != http.StatusTemporaryRedirect {
		t.Errorf("3-Post availability with empty request body (nil) gave wrong status code: got %d, wanted %d", rr.Code, http.StatusTemporaryRedirect)
	}
	...
	*****************************************/
	...
	// since we have rooms available, we expect to get status http.StatusTemporaryRedirect
	if rr.Code != http.StatusTemporaryRedirect {
		t.Errorf("4-Post availability with invalid start date gave wrong status code: got %d, wanted %d", rr.Code, http.StatusTemporaryRedirect)
	}
	...
	*****************************************/
	...
	// since we have rooms available, we expect to get status http.StatusTemporaryRedirect
	if rr.Code != http.StatusTemporaryRedirect {
		t.Errorf("5-Post availability with invalid end date gave wrong status code: got %d, wanted %d", rr.Code, http.StatusTemporaryRedirect)
	}
	...
	*****************************************/
	...
	// since we have rooms available, we expect to get status http.StatusTemporaryRedirect
	if rr.Code != http.StatusSeeOther {
		t.Errorf("6-Post availability when database query fails gave wrong status code: got %d, wanted %d", rr.Code, http.StatusSeeOther)
	}
}
...
func TestRepository_AvailabilityJSON(t *testing.T) {
	...
	*****************************************/
	...
	var j jsonResponse
	// get what the server is sending back and convert to JSON
	// data is the response body as  a slice of bytes, converted to a string and 
	// put into the JSON variable, j
...
func getCtx(req *http.Request) context.Context{
	...
	/* Load retrieves the session data for the given token from the session store, and returns a new context.
	Context containing the session data. If no matching token is found then this will create a new session.
